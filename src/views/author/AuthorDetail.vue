    ...
    <div class="top-container" v-if="!loading">
      <table>
        <tr>
          <td rowspan="3">
            <img
              v-if="author.attributes.image_path"
              :src="author.attributes.image_path"
              style="
                height: 90px;
                width: 90px;
                object-fit: cover;
     ...
     <div style="display:flex; justify-content:end; margin-right background:yellow; ">
      <CircularIconButton :icon="'delete'" @buttonPress="deleteAuthor" />
    </div>
...
export default {
  name: "AuthorDetail",
  // props:['author'],
  components: { UploadProfileImage,UploadProfileInfo,CircularIconButton },
  data() {
    return {
      author: {},
      loading: true,
